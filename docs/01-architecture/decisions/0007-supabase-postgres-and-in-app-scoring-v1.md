...
1. **Database wiring:** Prisma reads the integration's own env names: `POSTGRES_PRISMA_URL` (pooled, queries) and `POSTGRES_URL_NON_POOLING` (non-pooling, migrations via `directUrl`; the transaction pooler hangs prisma migrate, and connection failures with a bare schema engine error mean the free-tier database is suspended and needs a dashboard restore). No `DATABASE_URL` alias to maintain.
...
- The Clerk webhook remains unconfigured in production (`CLERK_WEBHOOK_SECRET` missing); the lazy user-upsert in the businesses API covers user creation until the owner adds the webhook in the Clerk dashboard. That lazy path originally wrote a shared placeholder email (`""`) on create, and `User.email` is `@unique` — the second distinct user to ever hit onboarding while the webhook stays unconfigured got a Prisma unique-constraint error on every subsequent attempt, surfacing as "Failed to save business" (500) in the UI. Fixed 2026-08-29: the fallback now fetches the real address via Clerk's `currentUser()` before creating the row (falling back to a per-user placeholder only if Clerk has none), and the write stays an `upsert` so a same-user double-submit race can't collide on the id either. Configuring the real webhook is still the more robust long-term fix and remains open.
