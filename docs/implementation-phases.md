...
- [ ] Owner actions that remain: **restore the suspended Supabase database** (Vercel dashboard, Storage tab, resource `supabase-champagne-nest`, open in Supabase, Restore; free-tier projects auto-pause after 7 idle days and this one has never served a query, so it is the root cause of every failed DB connection and of PR #2's red visibility-os preview); resolve the GitHub Actions billing lock so CI can run (GitHub Settings, Billing); configure or uninstall the Prisma GitHub App ("Prisma Compute Deploy" has been failing since before this PR; repo Settings, Integrations).
